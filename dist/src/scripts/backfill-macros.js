    ...
    for (const meal of missingMeals) {
        ...
        try {
            ...
            if (meal.source === "OPEN_FOOD_FACTS" && meal.externalId) {
                macrosPer100g = await (0, openfoodfacts_service_1.fetchOFFProduct)(meal.externalId);
            }
            ...
            if (!macrosPer100g && meal.name) {
                ...
                if (ciqualMatch) {
                    ...
                    // Try searching OFF
                    const offResults = await (0, openfoodfacts_service_1.searchOFFProducts)(meal.name);
