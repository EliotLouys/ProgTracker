...
async function importFatSecret() {
    ...
    const content = fs_1.default.readFileSync(path_1.default.resolve(filePath), "utf-8");
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!cleanLine || cleanLine.startsWith("Date,") || cleanLine.startsWith("\"Total"))
            continue;
        ...
        if (firstCol.startsWith("  ")) {
            ...
            if (name && kcalStr) {
                ...
                if (!isNaN(kcal) && kcal > 0) {
                    try {
                        await prisma_1.prisma.mealLog.create({
                            data: {
                                userId,
                                name: name,
                                source: "CUSTOM",
